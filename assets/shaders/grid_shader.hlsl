...
struct VertexOutput {
    ...
    float4 near_point : TEXCOORD0;
    float4 far_point : TEXCOORD1;
};
...
float3 unproject_point(float3 xyz, float4x4 inverse_view, float4x4 inverse_projection) {
    ...
    return unprojected_point.xzy / unprojected_point.w;
}
...
VertexOutput vs_main(
  uint vertex_id : SV_VertexID
    ...
    output.near_point = float4(near_point.xyz, 1.0);
    output.far_point = float4(far_point.xyz, 1.0);
    return output;
}
...
float compute_depth(float3 position, float4x4 view_projection) {
    const float4 clip_space_position = mul(view_projection, float4(position.xzy, 1.0));
